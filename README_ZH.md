...
NeoForge 1.21.1 的 Minecraft 物流模组。支持物品、流体、能量与联动资源传输，并提供可视化网络管理、跨维度链接、升级、过滤器、分组权限和蓝图功能。
...
- 按住可配置的“复选网络预览节点”组合键点击节点，可增减复选；按住该键拖动预览空白区域可框选节点，右击空白区域可清空选择。
- 拖动任一已选节点可整体移动当前选择；重新打开配置器时，预览会自动将当前网络居中。
...
ticker_batch_size = 50         # 每个服务器 tick 全局共享的节点/类型候选基数
ticker_time_budget_us = 1500   # 所有维度共享的每服务器 tick 物流调度软时间预算（微秒）
