...
public class NbtModeControls {
    ...
    public static String getModeTooltipKey(double mx, double my, int leftPos,
                                           int topPos, NbtMatchMode mode) {
        if (!isModeBtnHovered(mx, my, leftPos, topPos)) return null;
        return mode == NbtMatchMode.PARTIAL
            ? "gui.staticlogistics.part_match_button"
            : "gui.staticlogistics.full_match_button";
    }

    /**
     * 在界面其余提示完成后绘制，确保 NBT 模式提示位于最上层。
     */
    public static void renderTooltip(GuiGraphics g, Font font, int leftPos, int topPos,
                                     int mx, int my, NbtMatchMode mode) {
        String key = getModeTooltipKey(mx, my, leftPos, topPos, mode);
        if (key != null) g.renderTooltip(font, Component.translatable(key), mx, my);
    }
