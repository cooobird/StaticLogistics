...
public class NodeConfiguratorScreen extends AbstractConfiguratorScreen<NodeConfiguratorMenu> {
    ...
    protected void init() {
        ...
        priorityBox = makeBox(RARITY_BOX_X, ROW3_Y, "priority", true,
            menu.getPriority(), "-?[0-9]*", 10);
        keepStockBox = makeBox(STOCK_BOX_X, ROW4_Y, "keepStock", false,
    ...
    private EditBox makeBox(int x, int y, String labelKey, boolean priority,
                            int init, String pattern, int maxLen) {
        EditBox b = new EditBox(font, leftPos + x, topPos + y, EDIT_BOX_W, BTN,
            ...
            try {
                ...
                if (v != current) {
                    send(new FaceConfigurationEdit.NumberEdit(
                        priority ? FaceConfigurationEdit.NumberField.PRIORITY
    ...
    public boolean mouseClicked(double mx, double my, int button) {
        ...
        if (NodeConfigControls.hitToggle(mx, my, lx + INPUT_TOGGLE_X, ty + ROW1_Y)) {
            send(new FaceConfigurationEdit.BooleanEdit(
        ...
        if (NodeConfigControls.hitChannel(mx, my, lx + INPUT_CHANNEL_X, ty + CHANNEL_Y)) {
            cycleCh(FaceConfigurationEdit.ChannelField.INPUT, menu.getInputChannel(), button);
            return true;
        }
        if (NodeConfigControls.hitChannel(mx, my, lx + OUTPUT_CHANNEL_X, ty + CHANNEL_Y)) {
            cycleCh(FaceConfigurationEdit.ChannelField.OUTPUT, menu.getOutputChannel(), button);
            return true;
        }
        ...
        if (menu.isGlobalOutputEnabled()) {
            if (NodeConfigControls.hitCycleBtn(mx, my, lx + STRATEGY_X, ty + STRATEGY_Y, menu.getStrategy().getDisplayName(), font)) {
                ...
                send(new FaceConfigurationEdit.StrategyEdit(vs.get(n)));
                playClickSound();
                return true;
            }
            if (NodeConfigControls.hitCycleBtn(mx, my, lx + EXTRACTION_X, ty + EXTRACTION_Y, menu.getExtractionMode().getDisplayName(), font)) {
                ...
                send(new FaceConfigurationEdit.ExtractionEdit(vs[n]));
                playClickSound();
                return true;
            }
        }
    ...
    private void cycleCh(FaceConfigurationEdit.ChannelField field, int cur, int btn) {
        int n = btn == 1 ? (cur - 1 < 1 ? 16 : cur - 1) : (cur + 1 > 16 ? 1 : cur + 1);
        send(new FaceConfigurationEdit.ChannelEdit(field, n));
        playClickSound();
    }
    ...
    private void syncType() {
        send(new FaceConfigurationEdit.SelectedTypesEdit(menu.getSelectedTypeIds()));
    }
}
