...
import com.coobird.staticlogistics.client.gui.component.NodeConfigControls;
...
import com.coobird.staticlogistics.network.c2s.C2SOpenNodeFilterPayload;
...
public class NodeConfiguratorScreen extends AbstractConfiguratorScreen<NodeConfiguratorMenu> {
    ...
    private void renderOverlayTooltips(GuiGraphics g, int mx, int my) {
        ...
        if (in) {
            ...
            if (NodeConfigControls.hitOpBtn(mx, my, bx, by) || NodeConfigControls.hitOpBtn(mx, my, bx + BTN + 2, by))
                g.renderTooltip(font, Component.translatable("gui.staticlogistics.priority.tooltip",
                    SLKeyMappings.PRIORITY_X10.getTranslatedKeyMessage(),
                    SLKeyMappings.PRIORITY_X5.getTranslatedKeyMessage()), mx, my);
        }
    ...
    private void adjPriority(int d) {
        if (d == 0) return;
        d *= getAdjustmentMultiplier();
        send(new FaceConfigurationEdit.NumberEdit(
            FaceConfigurationEdit.NumberField.PRIORITY, saturatedAdd(menu.getPriority(), d)));
    }
    ...
    private void adjKeepStock(int d) {
        ...
        send(new FaceConfigurationEdit.NumberEdit(
            ...
            Math.max(0, saturatedAdd(menu.getKeepStock(), d))));
    }

    private static int getAdjustmentMultiplier() {
        boolean x10Down = SLKeyMappings.isGuiKeyDown(SLKeyMappings.PRIORITY_X10);
        boolean x5Down = SLKeyMappings.isGuiKeyDown(SLKeyMappings.PRIORITY_X5);
        if (x10Down && x5Down) return 64;
        if (x10Down) return 10;
        if (x5Down) return 5;
        return 1;
    }
    ...
    private void send(String key, Object val) {
        FaceConfigurationEdit edit = switch (key) {
            case "globalInput" -> new FaceConfigurationEdit.BooleanEdit(
                FaceConfigurationEdit.BooleanField.GLOBAL_INPUT, (Boolean) val);
            case "globalOutput" -> new FaceConfigurationEdit.BooleanEdit(
                FaceConfigurationEdit.BooleanField.GLOBAL_OUTPUT, (Boolean) val);
            case "inputChannel" -> new FaceConfigurationEdit.ChannelEdit(
                FaceConfigurationEdit.ChannelField.INPUT, (Integer) val);
            case "outputChannel" -> new FaceConfigurationEdit.ChannelEdit(
                FaceConfigurationEdit.ChannelField.OUTPUT, (Integer) val);
            case "priority" -> new FaceConfigurationEdit.NumberEdit(
                FaceConfigurationEdit.NumberField.PRIORITY, (Integer) val);
            case "keep_stock" -> new FaceConfigurationEdit.NumberEdit(
                FaceConfigurationEdit.NumberField.KEEP_STOCK, Math.max(0, (Integer) val));
            case "strategy" -> new FaceConfigurationEdit.StrategyEdit(
                DistributionStrategyRegistry.byName((String) val));
            case "extractionMode" -> new FaceConfigurationEdit.ExtractionEdit(
                ExtractionMode.byName((String) val, ExtractionMode.SEQUENTIAL));
            default -> throw new IllegalArgumentException("Unknown face configuration field: " + key);
        };
        send(edit);
    }
    ...
    private void send(FaceConfigurationEdit edit) {
        SLNetwork.HANDLER.sendToServer(
            new C2SConfigureFacePayload(menu.getPos(), menu.getFace(), edit));
    }

    private void syncType() {
        SLNetwork.HANDLER.sendToServer(new C2SConfigureFacePayload(
            menu.getPos(), menu.getFace(),
            new FaceConfigurationEdit.SelectedTypesEdit(menu.getSelectedTypeIds())));
    }
}
