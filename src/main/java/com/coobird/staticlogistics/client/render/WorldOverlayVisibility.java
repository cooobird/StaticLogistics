 ...
 * 世界覆盖层的统一可见性判定。
 *
 * <p>{@link LevelRenderer#isChunkCompiled(BlockPos)} 直接复用原版当前区块渲染器的
...
public final class WorldOverlayVisibility {
    ...
    public boolean isBlockVisible(BlockPos position) {
        return levelRenderer.isChunkCompiled(position)
    ...
    public boolean isConnectionVisible(BlockPos first, BlockPos second) {
        if (!levelRenderer.isChunkCompiled(first) || !levelRenderer.isChunkCompiled(second)) {
            return false;
        }
