...
public class LinkWorldRenderer {
    ...
    public static final RenderType PIPE_XRAY = RenderType.create(
        "staticlogistics:pipe_xray", DefaultVertexFormat.POSITION_COLOR, VertexFormat.Mode.QUADS, 1536, false, false,
    ...
    public static void onRenderLevelStage(RenderLevelStageEvent event) {
        ...
        // 存点预览
        if (stack.getItem() instanceof LinkConfiguratorItem lci) {
            try {
                LinkConfiguratorItem.ToolSettings settings = lci.getSettings(stack);
                if (settings != null) {
                    boolean hasNodes = !settings.storedNodes().isEmpty();
                    boolean hasMode = settings.storedMode() != null;
                    if (hasNodes && hasMode) {
                        renderStoredNodes(settings, dim, mat, b, cam, maxD2, pulse);
                    }
                }
            } catch (Exception e) {
                System.out.println("[SL-Render] storedNodes render error: " + e.getMessage());
            }
        }
