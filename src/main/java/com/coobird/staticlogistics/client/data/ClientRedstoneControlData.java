...
public enum ClientRedstoneControlData {
    ...
    /**
     * 按检测点和模式归并当前客户端已知的控制集合，供世界覆盖层渲染。
     */
    public List<ControlGroup> getControlGroups(GroupKey groupKey) {
        ...
        grouped.forEach((binding, entries) -> {
            State state = entries.getFirst().getValue();
    ...
    /**
     * 首包先清除该物流分组的旧投影，后续分页按顺序补入当前绑定。
     */
    public void acceptGroupPage(com.coobird.staticlogistics.api.group.GroupKey groupKey,
                                boolean reset, List<GroupEntry> entries) {
    ...
    /**
     * 只刷新已有控制绑定的运行态，不改动绑定结构。
     * 结构低频同步、信号高频同步共用同一份客户端投影，避免旧轮询拖慢世界渲染。
     */
