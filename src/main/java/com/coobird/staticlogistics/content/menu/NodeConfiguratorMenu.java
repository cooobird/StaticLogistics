...
public class NodeConfiguratorMenu extends AbstractContainerMenu {
    ...
    private NodeConfiguratorMenu(int containerId, Inventory playerInventory,
        ...
        if (player.level() instanceof ServerLevel serverLevel && pos != null && face != null) {
            ...
            if (mgr != null) {
                ...
                filterHandler = faceConfig == null
                    ? new ItemStackHandler(FILTER_SLOTS) : faceConfig.filterConfig.getUpgrades();
    ...
    public void setInputChannel(int channel) {
        if (faceConfig != null && faceConfig.linkConfig.getInputChannel() != channel) {
            faceConfig.setInputChannel(channel);
            syncFaceSlots();
        }
    }
    ...
    public void setOutputChannel(int channel) {
        if (faceConfig != null && faceConfig.linkConfig.getOutputChannel() != channel) {
            faceConfig.setOutputChannel(channel);
            syncFaceSlots();
        }
    }
    ...
    public void syncFaceSlots() {
        ...
        if (player.level() instanceof ServerLevel level && pos != null && face != null) {
            ...
            if (snapshot != null) {
                ...
                inputChannelSlot.set(snapshot.inputChannel());
                outputChannelSlot.set(snapshot.outputChannel());
    ...
    private void addPlayerInventory(Inventory inv) {
        int x = (MenuLayout.BACKGROUND_WIDTH - MenuLayout.INVENTORY_WIDTH) / 2 + 7;
        int ty = MenuLayout.BACKGROUND_HEIGHT + 8;
            ...
            if (stack.getItem() instanceof UpgradeItem upg) {
                for (int i = 0; i < TOTAL_CONFIG_SLOTS; i++) {
                    ...
                    if (cfgSlot.mayPlace(stack)) {
                        ...
                        if (existing.isEmpty() || (ItemStack.isSameItemSameTags(existing, stack) && existing.getCount() < cfgSlot.getMaxStackSize(stack))) {
    ...
    public boolean stillValid(Player player) {
        if (!(player instanceof net.minecraft.server.level.ServerPlayer serverPlayer)
            || pos == null || face == null || !(player.level() instanceof ServerLevel level)) {
