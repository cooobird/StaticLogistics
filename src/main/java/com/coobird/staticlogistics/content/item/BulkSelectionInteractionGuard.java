...
/**
 * 记录由批量选点数据包声明的右击，阻止同一次原版交互继续打开容器或配置界面。
 */
public final class BulkSelectionInteractionGuard {
    private static final long MAX_AGE_TICKS = 2L;
    private static final Map<MinecraftServer, Map<UUID, Interaction>> INTERACTIONS =
        new WeakHashMap<>();
    ...
    public static void mark(ServerPlayer player, BlockPos pos, Direction face) {
        ...
        INTERACTIONS.computeIfAbsent(player.server, ignored -> new HashMap<>())
            .put(player.getUUID(), new Interaction(
                pos.immutable(), face, player.serverLevel().getGameTime()));
    }
    ...
    public static boolean matches(ServerPlayer player, BlockPos pos, Direction face) {
        ...
        Map<UUID, Interaction> serverInteractions = INTERACTIONS.get(player.server);
        if (serverInteractions == null) return false;
        Interaction interaction = serverInteractions.get(player.getUUID());
        if (interaction == null) return false;
        long age = player.serverLevel().getGameTime() - interaction.gameTime();
        if (age < 0L || age > MAX_AGE_TICKS) {
            serverInteractions.remove(player.getUUID());
            return false;
        }
