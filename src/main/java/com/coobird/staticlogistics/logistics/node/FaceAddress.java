 ...
 * 单维度内无损的面地址。
 *
 * <p>方块位置本身已经占满 64 位，方向不能再安全塞进同一个 {@code long}。
 * 因此运行时仓储、缓存、版本和脏集合统一使用这个复合键。
 */
public record FaceAddress(long posLong, Direction face) {
    ...
    public static FaceAddress parseStorageKey(String value) {
        ...
        BlockPos legacyPos = BlockPos.of(legacyKey >> 3);
        Direction legacyFace = Direction.from3DDataValue(legacyFaceId);
        return of(legacyPos, legacyFace);
    }
}
