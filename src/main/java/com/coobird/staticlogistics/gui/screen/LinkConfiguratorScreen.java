...
public class LinkConfiguratorScreen extends Screen {
    ...
    public boolean mouseClicked(double mx, double my, int button) {
        ...
        if (listResult != null) {
            switch (listResult.getAction()) {
                ...
                case DELETE -> {
                    SLNetwork.HANDLER.sendToServer(new C2SDeleteGroupPayload(listResult.getGroupId()));
                    var player = Minecraft.getInstance().player;
                    if (player != null) {
                        ClientLinkData.INSTANCE.removeKnownGroup(player.getUUID(), listResult.getGroupId());
                        ClientLinkData.INSTANCE.removeServerEmptyGroup(player.getUUID(), listResult.getGroupId());
                    }
                    syncSettings("", true);
                }
            }
    ...
    private void handleConfirmRename() {
        ...
        if (!Objects.equals(editingId, newId)) {
            SLNetwork.HANDLER.sendToServer(new C2SGroupRenamePayload(editingId, newId));
            var player = Minecraft.getInstance().player;
            if (player != null)
                ClientLinkData.INSTANCE.removeKnownGroup(player.getUUID(), editingId);
            if (PortItemStackExtension.getDataOrDefault(stack, SLDataComponents.SELECTED_GROUP.get(), "").equals(editingId))
