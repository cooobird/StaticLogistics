...
public class TransferRegistries {
    private static final Map<ResourceLocation, TransferType> TYPES = new ConcurrentHashMap<>();
    private static final Map<ResourceLocation, ITransferHandler> HANDLERS = new ConcurrentHashMap<>();
    ...
    public static void init() {
        ITEM = registerInternal("item", 0xFFFFFFFF, 0, Capabilities.ItemHandler.BLOCK, SLConfig.getItemStack());
        FLUID = registerInternal("fluid", 0xFF3366FF, 1, Capabilities.FluidHandler.BLOCK, SLConfig.getFluidStack());
        ENERGY = registerInternal("energy", 0xFFFFFF00, 2, Capabilities.EnergyStorage.BLOCK, SLConfig.getEnergyStack());
    ...
    private static TransferType registerInternal(String name, int color, int offset, BlockCapability<?, Direction> cap, int size) {
        ResourceLocation id = Staticlogistics.asResource(name);
        TransferType type = new TransferType(id, color, offset, "transfer_type.staticlogistics." + name, cap, size);
        TYPES.put(id, type);
        return type;
    }
